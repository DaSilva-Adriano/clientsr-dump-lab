...
Not implemented (on purpose): NVIDIA RTX VSR, bicubic, Lanczos, Infuse, MetalFX, Topaz, Real-ESRGAN x4plus, any 1.5× path.
...
Each MP4 gets a sidecar `*.dump.json` (source path, WxH in/out, fps, token, device, backend command, ffmpeg command, UTC start/end, elapsed seconds, output bytes). After a batch, `dump_manifest.csv` is written in the output folder.
